...
def test_analyze_cashback_categories_success(
    mock_get_data_time, mock_get_cashback, mock_get_path, setup_test_file
):
...
def test_analyze_cashback_categories_invalid_year(mock_logger, setup_test_file):
    ...
    with pytest.raises(Exception):
        analyze_cashback_categories(
            setup_test_file, datetime.now().year + 1, 1
        )  # Год больше текущего
...
def test_analyze_cashback_categories_empty_file(tmp_path):
    ...
    with patch("src.services.setup_function_logger"):
        ...
        assert (
            "пуст" in str(exc_info.value).lower()
            or "не найден" in str(exc_info.value).lower()
        )
