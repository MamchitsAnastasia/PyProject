...
def test_main_info_success(valid_date: str, mock_data: MagicMock) -> None:
    ...
    with (
        patch("src.views.get_time_for_greeting", return_value=mock_data["greeting"]),
        patch(
            "src.views.get_data_time",
            return_value=["01.10.2023 00:00:00", "15.10.2023 12:00:00"],
        ),
...
def test_main_info_partial_data(valid_date: str, mock_data: MagicMock) -> None:
    ...
    with (
        patch(
            "src.views.get_time_for_greeting",
            return_value=partial_mock_data["greeting"],
        ),
        patch(
            "src.views.get_data_time",
            return_value=["01.10.2023 00:00:00", "15.10.2023 12:00:00"],
