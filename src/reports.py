...
def spending_by_category(path_to_file: str, category: str, date: Optional[str] = None) -> DataFrame:
    """Функция принимает файл формата xlsx, название категории и дату, и возвращает файл с операциями по категории за последние 3 месяца"""
    logger = None
    try:
        ...
        if not path_to_file.strip() or not category.strip():
            logger.error("Параметры path_to_file и category не могут быть пустыми строками")
