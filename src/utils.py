...
def get_path_and_period(path_to_file: str, period_date: list) -> DataFrame:
    ...
    try:
        ...
        try:
            ...
            )  # Перевожу в формат datetime, ранжирую по дню
...
def get_card_with_spend(sorted_df: DataFrame) -> list[dict]:
    ...
    try:
        ...
        try:
            card_spend_transactions: list[dict] = []
            ...
            for index, row in card_sorted.iterrows():
                try:
                    if row["Сумма операции"] < 0:
                        ...
                        card_spend_transactions.append(transaction)
...
def get_currency(path_to_json: str) -> list[dict]:
    ...
    try:
        ...
        currency_rates: list[dict[str, str]] = []
        try:
            ...
            raise json.JSONDecodeError(
                f"Ошибка декодирования JSON: {str(json_err)}",
                json_err.doc,
                json_err.pos,
            ) from json_err
...
def get_cashback_by_category(categories_for_month: DataFrame) -> dict[str, float]:
    ...
    try:
        ...
        try:
            categories_sorted = categories_for_month[
                [
                    "Сумма операции",
                    "Кэшбэк",
                    "Категория",
