...
from src.utils import get_path_and_period, get_data_time, get_cashback_by_category, setup_function_logger
...
def analyze_cashback_categories(path_to_file: str, year: int, month: int) -> DataFrame:
    ...
    try:
        ...
        logger.info(f"Начало выполнения функции с параметрами: path_to_file={path_to_file}, year={year}, month={month}")
        ...
        try:
            ...
            try:
                categories_for_month = get_path_and_period(PATH_TO_FILE, time_period) # Делаю срез таблицы по этому периоду
                try:
                    ...
                    raise Exception(f"Ошибка при расчете кэшбэка по категориям: {str(e)}") from e

            except Exception as e:
                logger.error(f"Ошибка при фильтрации данных по периоду: {str(e)}")
                raise Exception(f"Ошибка при фильтрации данных по периоду: {str(e)}") from e
